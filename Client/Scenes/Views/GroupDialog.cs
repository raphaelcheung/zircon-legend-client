    ...
    public sealed  class GroupDialog : DXWindow
    {
        ...
        public GroupDialog()
        {
            ...
            AddButton.MouseClick += (o, e) =>
            {
                ...
                {
                    GameScene.Game.ReceiveChat("队伍数量已达极限", MessageType.System);
                    return;
                }

                if (Members.Count >= Globals.GroupLimit)
                {
                    GameScene.Game.ReceiveChat("你不是队伍领袖.", MessageType.System);
                    return;
                }

                DXInputWindow window = new DXInputWindow("请输入你要邀请入队者的名称.", "邀请入队")
                {
