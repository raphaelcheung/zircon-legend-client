...
namespace Client.Scenes
{
    public sealed class SelectScene : DXScene
    {
        ...
        public SelectDialog SelectBox { get; set; }
        ...
        public sealed class SelectDialog : DXWindow
        {
            ...
            private void DeleteButton_MouseClick(object sender, MouseEventArgs e)
            {
                ...
                DXMessageBox box = new DXMessageBox($"确定要删除角色 {character.CharacterName} 吗？\n" +
                                                    $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 秒 后再确认.", "删除角色", DXMessageBoxButtons.YesNo);
                ...
                box.ProcessAction = () =>
                {
                    if (CEnvir.Now > deleteTime)
                    {
                        ...
                        box.Label.Text = $"确定要删除角色 {character.CharacterName} 吗？\n" +
                                         $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 秒后进行确认.";
                };
            }
        ...
        public sealed class NewCharacterDialog : DXWindow
        {
            ...
            public void OnSelectedClassChanged(MirClass oValue, MirClass nValue)
            {
                ...
                Type type = SelectedClass.GetType();
                MemberInfo[] infos = type.GetMember(SelectedClass.ToString());
                DescriptionAttribute description = infos[0].GetCustomAttribute<DescriptionAttribute>();

                SelectedClassLabel.Text = description?.Description ?? "";
            }
            ...
            public void OnSelectedGenderChanged(MirGender oValue, MirGender nValue)
            {
                ...
                Type type = SelectedGender.GetType();
                MemberInfo[] infos = type.GetMember(SelectedGender.ToString());
                DescriptionAttribute description = infos[0].GetCustomAttribute<DescriptionAttribute>();

                SelectedGenderLabel.Text = description?.Description ?? "";
            }
            ...
            public NewCharacterDialog()
            {
                ...
                CharacterNameHelpLabel = new DXLabel
                {
                    ...
                    Parent = this,
                    Text = "[?]",
                    Hint = $"角色名称.\n有效字符: a-z A-Z 0-9.\n长度: 介于 {Globals.MinCharacterNameLength} 和 {Globals.MaxCharacterNameLength} 之间.\n可以在同一帐户上使用以前的名称.",
                };
        ...
        public sealed class SelectButton : DXControl
        {
            ...
            public void OnSelectInfoChanged(SelectInfo oValue, SelectInfo nValue)
            {
                ...
                Type type = SelectInfo.Class.GetType();
                MemberInfo[] infos = type.GetMember(SelectInfo.Class.ToString());
                DescriptionAttribute description = infos[0].GetCustomAttribute<DescriptionAttribute>();
                ClassLabel.Text = description?.Description ?? "";
